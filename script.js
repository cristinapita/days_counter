document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateCountdown(dates, currentTime, isCurrent) {
        ...
        let days = Math.floor(remainingSeconds / (3600 * 24)) - 1; // Restar un día
        let hours = Math.floor((remainingSeconds % (3600 * 24)) / 3600) - 1; // Restar una hora
        ...
        if (currentDate > newYearDate) {
            ...
            days = daysInNewMonth - daysOverflow - 1; // Restar un día
        } else {
            days = newYearDate - currentDate - 1; // Restar un día
        }

        // Ajustar horas restantes
        if (hours < 0) {
            hours += 24;
        }
